...
impl Day5 {
  ...
  where P: AsRef<Path>, {
    ...
    maps.push_back(Self::parse_ranges(&path_to_input, "temperature-to-humidity"));
    ...
    let Some(mut answer) = answers.pop() else { panic!("No results!") };
    for a in answers {
      if a < answer {
        answer = a;
      }
  ...
  fn apply_chain(seed: u64, maps: &VecDeque::<VecDeque::<(u64, u64, u64)>>) -> u64 {
    maps.iter().fold(seed, |acc, map| Self::find_mapped_value(acc, map))
  }

  fn find_mapped_value(val1: u64, map: &VecDeque::<(u64, u64, u64)>) -> u64 {
    let ret = map.iter().fold(val1, |acc, (val1_start, range, val2_start)| 
  ...
  where P: AsRef<Path>, {
    ...
    let Some(seeds) = first_line.split(": ").collect::<VecDeque<&str>>().pop_back() else {
      panic!("No seeds!");
    };
