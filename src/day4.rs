      ...
      for line in lines {
        if let Ok(ip) = line {
          ...
          for winning_number_match in re.find_iter(winning_numbers_txt) {
            winning_numbers.insert(winning_number_match.as_str().to_string().parse::<i32>().unwrap());
          }
          for number_match in re.find_iter(numbers_i_have_txt) {
            numbers_i_have.push(number_match.as_str().to_string().parse::<i32>().unwrap());
          }
        ...
        if let Ok(ip) = line {
          ...
          numbers_i_have.iter().fold(id, |acc, x| 
            if winning_numbers.contains(x) {
              ...
              let mut existing_copies_of_next_card = match num_copies_per_card.get(&win_new_card_id) {
                Some(&num_cards) => num_cards,
                _ => 0
              };
              existing_copies_of_next_card = existing_copies_of_next_card + existing_copies_of_this_card;
              num_copies_per_card.insert(win_new_card_id, existing_copies_of_next_card);
              win_new_card_id
            } else {
              acc
            }
